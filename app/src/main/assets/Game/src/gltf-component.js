const gltf_component = (() => {
	...
	class StaticModelComponent extends entity.Component {
		...
		_OnLoaded(obj) {
			...
			this._target.position.copy(this._parent ? this._parent._position : new THREE.Vector3(0, 0, 0));
	...
	class AnimatedModelComponent extends entity.Component {
		...
		_OnLoaded(obj, animations) {
			...
			this.Broadcast({
				...
				value: this._parent ? this._parent._position : new THREE.Vector3(0, 0, 0),
			});
			...
			if (this._parent) this._parent._mesh = this._target;
			this.Broadcast({
