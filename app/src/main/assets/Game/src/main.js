...
class HackNSlashDemo {
	...
	_LoadPlayer() {
		...
		camera.AddComponent(new third_person_camera.ThirdPersonCamera({
			camera: this._camera,
	...
	createJoystick(dx, dy) {
		const _VS = [
			'void main() {',
			'	vec3 p = position;',
			'	vec3 pos = vec3(p.x ' + dx + ', p.y ' + dy + ', p.z);',
		...
		setInterval(() => joystick.position.copy(this._entityManager.Get('player')._position), 100)
		this._scene.add(joystick)
	}

	playMusic() {
		if (this.actx) return
		var notes = [300, 400, 500, 600]
		var music = [0, 1, 2, 3, 2, 0, 3, 1, 0, 2]
		this.actx = new AudioContext()
		var actx = this.actx
		var ton = actx.createOscillator()
		ton.type = 'sawtooth'
		...
		var i = 0
		setInterval(() => {
			i = i < music.length - 1 ? i + 1 : 0
			ton.frequency.value = notes[ music[i] ]
		}, 500)
	}
}
...
setTimeout(() => {
	...
	_APP.createJoystick('- 0.6', ' - 0.3')
	_APP.createJoystick('- 0.6', ' - 0.7')
	_APP.createJoystick('- 0.8', ' - 0.5')
	_APP.createJoystick('- 0.4', ' - 0.5')
	_APP.createJoystick('+ 0.7', ' - 0.5')
}, 2000)
